          ...
          {visible.map((certificate, index) => (
            ...
            >
              <div className="flex items-start justify-between gap-4">
                <span className="issuer-mark">{certificate.initials}</span>
              ...
              <div className="mt-auto flex items-center justify-between border-t border-line pt-4">
                ...
                {certificate.year ? <span className="font-mono text-[9px] text-accent">{certificate.year}</span> : <ExternalLink aria-hidden className="h-3 w-3 text-line" />}
              </div>
            </motion.article>
          ))}
        </motion.div>
